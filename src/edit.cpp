...
static uptr<Reaction> node_insert_remove_combiner(const Do<InsertNode>& a, const Do<RemoveNode>& b)
{
	if (a.node == b.node)
		return std::make_unique<Annihilation>();
	return {};
}
static uptr<Reaction> text_unmerge_insert_combiner(const Do<UnmergeText>& a, const Do<InsertText>& b)
{
